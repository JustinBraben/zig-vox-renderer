...
const Base = Dispatch.Base;
const Instance = Dispatch.Instance;
...
pub const GraphicsContext = struct {
    ...
    pub fn init(allocator: Allocator, app_name: [*:0]const u8, window: glfw.Window) !GraphicsContext {
        ...
        self.vkb = try Base.load(@as(vk.PfnGetInstanceProcAddr, @ptrCast(&glfw.getInstanceProcAddress)));
        ...
        self.vki = try Instance.load(self.instance, self.vkb.dispatch.vkGetInstanceProcAddr);
...
fn initializeCandidate(allocator: Allocator, vki: Instance, candidate: DeviceCandidate) !vk.Device {
...
fn allocateQueues(vki: Instance, pdev: vk.PhysicalDevice, allocator: Allocator, surface: vk.SurfaceKHR) !?QueueAllocation {
...
fn checkSurfaceSupport(vki: Instance, pdev: vk.PhysicalDevice, surface: vk.SurfaceKHR) !bool {
