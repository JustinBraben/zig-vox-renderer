...
pub fn setVec2f(self: Shader, name: [*c]const u8, value: [2]f32) void {
    gl.uniform2f(gl.getUniformLocation(self.ID, name), value[0], value[1]);
}
...
pub fn setVec4f(self: Shader, name: [*c]const u8, value: [4]f32) void {
    gl.uniform4f(gl.getUniformLocation(self.ID, name), value[0], value[1], value[2], value[3]);
}

pub fn setMat2f(self: Shader, name: [*c]const u8, value: [8]f32) void {
    const matLoc = gl.getUniformLocation(self.ID, name);
    gl.uniformMatrix2fv(matLoc, 1, gl.FALSE, &value);
}

pub fn setMat3f(self: Shader, name: [*c]const u8, value: [12]f32) void {
    const matLoc = gl.getUniformLocation(self.ID, name);
    gl.uniformMatrix2fv(matLoc, 1, gl.FALSE, &value);
}
